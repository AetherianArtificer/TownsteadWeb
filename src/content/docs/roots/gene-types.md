...
| Abilities and attributes | `pheno:ability`, `pheno:active_ability`, `pheno:attribute`, `pheno:flight_speed`, `pheno:step_height`, `pheno:buoyancy`, `pheno:wade`, `pheno:entity_group`, `pheno:effect_immunity`, `pheno:stuck_immunity`, `pheno:disable_regen` |
...
Resource genes can also be declared inside another gene's `resources` block as companion resources, and supporting components of any gene type can be declared inside a `companions` block. See [Gene Files](/roots/gene-files/) for both patterns, and [Resource HUD](/roots/resource-hud/) for meter presentation.
